...
static void dhcpv6_send(enum dhcpv6_msg req_msg_type, uint8_t trid[3], uint32_t ecs)
{
	...
	// Build IA_PDs
	size_t ia_pd_entry_count = 0, ia_pd_len = 0;
	uint8_t *ia_pd;
	struct odhcp6c_entry *pd_entries = odhcp6c_get_state(STATE_IA_PD, &ia_pd_entry_count);
	ia_pd_entry_count /= sizeof(*pd_entries);

	if (req_msg_type == DHCPV6_MSG_SOLICIT || (req_msg_type == DHCPV6_MSG_REQUEST && ia_pd_entry_count == 0 && pd_mode != IA_MODE_NONE)) {
		...
		ia_pd = alloca(ia_pd_entry_count * (sizeof(struct dhcpv6_ia_prefix) + 10 +
					sizeof(struct dhcpv6_ia_hdr)));

		for (size_t i = 0; i < ia_pd_entry_count; ++i) {
			...
			for (size_t j = i; j < ia_pd_entry_count; j++) {
				if (pd_entries[j].iaid != iaid)
					continue;
	...
	// Build IA_NAs
	size_t ia_na_entry_count, ia_na_len = 0;
	void *ia_na = NULL;
	struct odhcp6c_entry *ia_entries = odhcp6c_get_state(STATE_IA_NA, &ia_na_entry_count);
	ia_na_entry_count /= sizeof(*ia_entries);
	...
	struct dhcpv6_ia_addr ia_na_array[ia_na_entry_count];
	for (size_t i = 0; i < ia_na_entry_count; ++i) {
		ia_na_array[i].type = htons(DHCPV6_OPT_IA_ADDR);
		ia_na_array[i].len = htons(sizeof(ia_na_array[i]) - 4U);
...
static unsigned int dhcpv6_calc_refresh_timers(void)
{
	...
	size_t ia_na_entry_count, ia_pd_entry_count, i;
	size_t invalid_entries = 0;
	int64_t l_t1 = UINT32_MAX, l_t2 = UINT32_MAX, l_t3 = 0;

	ia_entries = odhcp6c_get_state(STATE_IA_NA, &ia_na_entry_count);
	ia_na_entry_count /= sizeof(*ia_entries);

	for (i = 0; i < ia_na_entry_count; i++) {
	...
	pd_entries = odhcp6c_get_state(STATE_IA_PD, &ia_pd_entry_count);
	ia_pd_entry_count /= sizeof(*pd_entries);

	for (i = 0; i < ia_pd_entry_count; i++) {
	...
	if (ia_pd_entry_count + ia_na_entry_count - invalid_entries) {
		t1 = l_t1;
		t2 = l_t2;
		t3 = l_t3;
	...
	return (unsigned int)(ia_pd_entry_count + ia_na_entry_count);
}
